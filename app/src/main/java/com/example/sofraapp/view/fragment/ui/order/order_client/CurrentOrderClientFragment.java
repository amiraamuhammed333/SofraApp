...
public class CurrentOrderClientFragment extends BaseFragment {
    View view;
    Unbinder unbinder;
    ...
    @BindView(R.id.current_order_client_recycleview)
    RecyclerView currentOrderClientRecycleview;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.current_order_client, container, false);
    ...
    private void iniRecyclerView() {
        ...
        currentOrderClientRecycleview.setLayoutManager(linearLayoutManager);
        onEndless = new OnEndless(linearLayoutManager, 1) {
            @Override
            public void onLoadMore(int current_page) {
                if (maxPage != 0) { getCurrentOrder(current_page); }
                if (current_page <= maxPage) {
                    if (maxPage != 0 && current_page != 1) {
                        previousPage = current_page;
                        if (filter) { }
                        else { getCurrentOrder(current_page); }
                    } else { onEndless.current_page = previousPage; }
                } else { onEndless.current_page = previousPage; } }};

        currentOrderClientRecycleview.addOnScrollListener(onEndless);
        currentOrderClientAdapter= new OrderClientAdapter(getActivity(),getActivity(),currentOrderClientDataList);
        currentOrderClientRecycleview.setAdapter(currentOrderClientAdapter);
        getCurrentOrder(1);
    }

    public void getCurrentOrder(int page){
        APIManger.getApis().getCurrentOrderClient("HRbqKFSaq5ZpsOKITYoztpFZNylmzL9elnlAThxZSZ52QWqVBIj8Rdq7RhoB"," current" ).
